...
def send_email_with_attachment(recipient_email, name, pdf_path,cc_emails=None):
    ...
    #         }
    #          # ✅ Add CC if provided (from Excel column)
    #         if cc_emails and str(cc_emails).strip().lower() != "nan":
    #             if "," in str(cc_emails):  # multiple CCs separated by comma
    #                 recipient_obj["cc"] = [{"email": email.strip()} for email in cc_emails.split(",")]
    #             else:  # single CC
    #                 recipient_obj["cc"] = [{"email": str(cc_emails).strip()}]
    #         ],
